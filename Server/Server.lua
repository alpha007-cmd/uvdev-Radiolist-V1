...
AddEventHandler("playerDropped", function()
	...
	for _, player in pairs(playersInCurrentRadioChannel) do
		if player.Source ~= src then
			TriggerClientEvent("uv-RadioList:Client:SyncRadioChannelPlayers", player.Source, src, 0, playersInCurrentRadioChannel)
		end
	end
...
function Connect(src, currentRadioChannel, radioChannelToJoin)
	...
	for _, player in pairs(playersInCurrentRadioChannel) do
		TriggerClientEvent("uv-RadioList:Client:SyncRadioChannelPlayers", player.Source, src, radioChannelToJoin, playersInCurrentRadioChannel)
	end
	playersInCurrentRadioChannel = {}
end
...
function Disconnect(src, currentRadioChannel) 
	local playersInCurrentRadioChannel = CreateFullRadioListOfChannel(currentRadioChannel)
	TriggerClientEvent("uv-RadioList:Client:SyncRadioChannelPlayers", src, src, 0, playersInCurrentRadioChannel)
	for _, player in pairs(playersInCurrentRadioChannel) do
		TriggerClientEvent("uv-RadioList:Client:SyncRadioChannelPlayers", player.Source, src, 0, playersInCurrentRadioChannel)
	end
	playersInCurrentRadioChannel = {}
...
if Config.LetPlayersSetTheirOwnNameInRadio then
    local function handleRadioNameChange(source, args, rawCommand, commandLength)
    ...
    RegisterCommand("nameinradio", function(source, args, rawCommand)
        handleRadioNameChange(source, args, rawCommand, string.len("nameinradio"))
    end)

    RegisterCommand("nmr", function(source, args, rawCommand)
        handleRadioNameChange(source, args, rawCommand, string.len("nmr"))
    end)
end
